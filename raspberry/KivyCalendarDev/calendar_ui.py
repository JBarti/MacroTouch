...
<DayButton>:
    ...
    gray: '#000000d0'
    purple: '#7f58c5'
    background_color: hex_to_color(self.gray)
    on_state: self.on_state
    canvas.before:
        Color: 
            rgb: hex_to_color(self.purple)
        BorderImage: 
            size: self.size
            pos: (self.x, self.center_y-(self.height/2-1))
...
class DatePicker(TextInput):
    """ 
    Date picker is ut textinput, if it focused shows popup with calendar
...
class CalendarWidget(RelativeLayout):
    ...
    def create_month_scr(self, month, toogle_today=False):
        ...
        for week in month:
            for day in week:
                isEnabled = bool(day[2])
                isWeekend = day[1] >= 5
                if isEnabled:
                    tbtn = DayButton(isWeekend, text=str(day[0]))
                else:
                    tbtn = DisabledDayLabel(text=str(day[0]))
    ...
    def prepare_data(self):
        ...
        # Set title
        self.title = "%s - %s" % (
            self.month_names[self.active_date[1] - 1],
            self.active_date[2],
        )
    ...
    def get_quarter(self):
        ...
        self.quarter_nums = cal_data.calc_quarter(
            self.active_date[2], self.active_date[1]
        )
        self.quarter = cal_data.get_quarter(self.active_date[2], self.active_date[1])
    ...
    def go_prev(self, inst):
        ...
        prev_scr_name = "%s-%s" % (self.month_names_eng[n], self.quarter_nums[0][0])
    ...
    def go_next(self, inst):
        ...
        next_scr_name = "%s-%s" % (self.month_names_eng[n], self.quarter_nums[2][0])
...
class DayButton(ToggleButton):
    def __init__(self, isWeekend, **kwargs):
        super(DayButton, self).__init__(**kwargs)
        self.background = "#8c0000dd" if isWeekend else "#000000d0"
        self.background_color = hex_to_color(self.background)

    def on_state(self, *args, **kwargs):
        state = args[1]
        if state == "down":
            self.background_color = hex_to_color("#00000000")
        else:
            self.background_color = hex_to_color(self.background)


class DisabledDayLabel(Label):
    pass
